        ...
        for file_path in file_list:
            ...
            for sheet_idx in range(1, src_wb.Worksheets.Count + 1):
                ...
                if include_header:
                    ...
                    current_row = copy_excel_fixed_range(
                        source_ws, dst_ws, header_start, header_end, current_row, paste_values
                    )
                    current_row += 1
                # Copy data rows (after header_end)
                current_row = copy_excel_range(source_ws, dst_ws, header_end + 1, current_row, paste_values)
                current_row += 1
                source_index += 1
...
def merge_multiple_files_openpyxl(file_list, output_path, keep_all_headers=True, header_start=1, header_end=1):
    if not file_list:
        return 0
        ...
        for sheet_name in src_wb.sheetnames:
            ...
            if include_header:
                ...
                current_row = copy_row_range_openpyxl(
                    source_ws, dst_ws, header_start, header_end, current_row
                ) + 1
            # Copy data rows (after header_end)
            real_last = get_last_nonempty_row_openpyxl(source_ws, header_end + 1)
            if real_last >= header_end + 1:
                current_row = copy_row_range_openpyxl(
                    source_ws, dst_ws, header_end + 1, real_last, current_row
                )
            current_row += 1
            source_index += 1
    ...
    if prefer_excel_com:
        try:
            return merge_multiple_files_excel_com(
                file_list, output_path, keep_all_headers, paste_values, header_start, header_end
            )
        except Exception as exc:
            if paste_values:
                raise
            print(f"Excel COM failed, falling back to openpyxl: {exc}")
    return merge_multiple_files_openpyxl(file_list, output_path, keep_all_headers, header_start, header_end)
...
class ExcelMergerApp(ctk.CTk):
    ...
    def setup_second_frame(self):
        ...
        self.files_keep_headers_var, self.files_paste_values_var, self.files_prefer_com_var = self.setup_merge_options(self.second_frame, 3, column=0, columnspan=2)
    ...
    def setup_third_frame(self):
        ...
        self.split_btn.grid(row=5, column=0, columnspan=2, pady=20)
        self.split_progress_frame, self.split_progress_bar, self.split_status_label = self._create_progress_widgets(self.third_frame, row=6, columnspan=2)
    ...
    def setup_guide_frame(self):
        ...
        guide_text = """Chào mừng bạn đến với Excel Toolbox!

Dưới đây là hướng dẫn chi tiết cách sử dụng các tính năng:

--- 1. GHÉP SHEET TRONG CÙNG 1 FILE ---
Tính năng này giúp bạn gộp dữ liệu từ nhiều sheet khác nhau trong 1 file Excel lại thành 1 sheet duy nhất (tên là TongHop).
• Bước 1: Bấm 'Chọn File' và trỏ tới file Excel của bạn.
• Bước 2: Chọn các sheet cần ghép từ danh sách hiện ra (có thể giữ Ctrl hoặc Shift để chọn nhiều, hoặc bấm 'Chọn tất cả Sheet').
• Bước 3: Cấu hình dòng tiêu đề và dữ liệu.
  - Tiêu đề: Khai báo dòng đầu và cuối của tiêu đề. Ví dụ: từ 1 đến 7.
  - Dữ liệu: Khai báo dòng bắt đầu và dòng kết thúc của dữ liệu. Nếu để kết thúc = 0, phần mềm tự động lấy tới dòng cuối cùng có chữ.
• Bước 4: Tùy chỉnh (Giữ giá trị, đánh lại STT...) và bấm 'Bắt đầu Ghép Sheet'.

--- 2. GHÉP NHIỀU FILE EXCEL ---
Tính năng này giúp bạn gộp toàn bộ các sheet từ NHIỀU file Excel khác nhau vào 1 file Excel duy nhất (hoặc 1 sheet duy nhất tuỳ tuỳ chọn).
• Bước 1: Bấm 'Thêm Files' và chọn các file bạn muốn ghép.
• Bước 2: (Tùy chọn) Chọn các file trong danh sách và bấm 'Xóa' nếu muốn bỏ bớt.
• Bước 3: Chọn các cài đặt: 'Giữ tiêu đề của mỗi sheet/file' hoặc 'Dán giá trị thay vì công thức'.
• Bước 4: Bấm 'Bắt đầu Ghép File' và chọn nơi lưu file kết quả.

--- 3. TÁCH FILE EXCEL ---
Tính năng này giúp bạn cắt nhỏ 1 sheet lớn thành nhiều sheet hoặc nhiều file nhỏ hơn.
• Bước 1: Bấm 'Chọn File' và chọn sheet cần tách.
• Bước 2: Chọn phương thức tách:
  - 'Theo giá trị cột': Tách các dòng có cùng một giá trị trong một cột cụ thể vào chung 1 nhóm. Bấm 'Tải lại cột' để cập nhật danh sách cột tiêu đề, sau đó chọn cột muốn làm tiêu chí tách.
  - 'Theo số lượng dòng': Tách đều mỗi phần chứa đúng X dòng. Bạn cần nhập số dòng vào ô 'Số dòng mỗi phần...'.
• Bước 3: Chọn hình thức xuất ra (Nhiều sheet trong cùng 1 file MỚI, hoặc xuất thành từng file riêng lẻ).
• Bước 4: Bấm 'Bắt đầu Tách File'.

--- LƯU Ý CHUNG ---
- Tùy chọn 'Ưu tiên Excel COM': Luôn khuyến khích bật vì nó giúp giữ nguyên ĐỊNH DẠNG (màu sắc, khung viền, độ rộng cột, dòng...) giống y hệt file gốc của bạn. Khi bật tuỳ chọn này, phần mềm sẽ mượn trình Excel trên máy bạn chạy ngầm để xử lý."""
        ...
        try:
            ...
            def task():
                num_parts = split_excel(
                    ...
                    column_index=column_index, row_count=row_count,
                )
